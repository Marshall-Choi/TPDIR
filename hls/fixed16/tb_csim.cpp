...
// symlink: images.h → ../../artifacts/hls/shared/images.h
#include "images.h"
...
int main() {
    ...
    printf("MNIST fixed16 C-sim — %d images from images.h\n", n);
    printf("label = i %% 10  (interleaved digit order from hls_export)\n\n");

    // images.h order: IMAGES[i] = bucket[i % 10][i // 10]  (see hls_export.export_shared)
    for (int i = 0; i < n; ++i) {
